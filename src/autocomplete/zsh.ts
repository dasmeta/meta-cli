...
export default class ZshCompWithSpaces {
  ...
  public generate(): string {
      ...
      for (const arg of firstArgs) {
        if (this.coTopics.includes(arg.id)) {
          ...
          if (cmd) {
            ...
            else if (Object.keys(cmd.flags).length > 0) {
    ...
    const genArgsCompBlock = (command: CommandCompletion) => {
      ...
      let block = '';
      let iterator = 0;
      for(const item of cmd.autocompleteArgs) {
        let prevArgs = '';
        block += `\t\t${iterator + 2})\n`;

        if(iterator === 0 && Object.keys(command.flags).length > 0) {
          block += '\t\t\tif [[ "${words[CURRENT]}" == -* ]]; then\n';
          block += `\t\t\t${this.genZshFlagArgumentsBlock(command.flags)}\n`
          block += '\t\t\t\treturn\n';
          block += '\t\t\tfi\n\n';
        }

        if(iterator > 0) {
          for(let i = 2; i < iterator + 2; i++) {
            prevArgs += (' ${line['+(i)+']}');
          }
        }

        block += `\t\t\tlocal -a ${item.arg}\n`;
        block += `\t\t\t${item.arg}=( $(${item.cmd}${prevArgs}) )\n`;
        block += `\t\t\t_describe "${item.arg}" ${item.arg}\n`;
        block += '\t\t\t;;\n'
        iterator++;
      }

      return `
_${this.config.bin}_${command.id}() {
  local prevArg=$words[CURRENT-1]
  
  # If previous word is a flag, adjust current position for proper completion
  if [[ "$prevArg" == --* ]]; then
      CURRENT=$((CURRENT - 1))
  fi

  # Check the current argument position
  case $CURRENT in
${block}
  esac
}
...
${this.commands.map(c => genArgsCompBlock(c)).filter(Boolean).join('\n')}
